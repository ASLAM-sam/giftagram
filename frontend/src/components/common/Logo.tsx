...
export const Logo: React.FC<LogoProps> = ({ className = '', isFooter = false }) => {
  ...
  return (
    <Link to="/" className={`inline-flex flex-col items-start group select-none ${className}`}>
      <span className="font-serif text-2xl md:text-[1.65rem] tracking-[0.18em] text-espresso-900 font-medium uppercase group-hover:text-rose-600 transition-colors duration-300">
        {name}
      </span>
      <span className="font-sans text-[0.65rem] tracking-[0.25em] text-rose-500 uppercase -mt-0.5 font-medium pl-0.5">
